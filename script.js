...
function renderCategorySelector() {
  ...
  if (!selector) {
    ...
    if (actions) {
      const reelsBtn = document.getElementById("openReelsBtn");
      if (reelsBtn) actions.insertBefore(selector, reelsBtn);
      else actions.appendChild(selector);
    } else if (homeHeader) homeHeader.appendChild(selector);
...
function closeReels() {
  document.getElementById("reelsSpace")?.classList.add("hidden");
...
function setCreateMode(mode) {
  ...
  const ta = document.getElementById("postText");
  if (ta) {
    ta.placeholder = state.createMode === "reel"
      ? "Write a caption for your Reel..."
      : "What's on your mind? Tag with @username";
  }
  // hint on media button
  const mediaLabel = document.querySelector('label[for="mediaInput"], .create-tools label, #mediaPickLabel');
  ...
  if (tools) {
    ...
    hint.textContent = state.createMode === "reel"
      ? "Reel mode: pick a short video (mp4/mov, under ~40MB)."
      : "";
  }
}
...
async function publishReel() {
  if (!currentUser) return alert("Please log in first.");
  const input = document.getElementById("reelVideoInput") || document.getElementById("mediaInput");
  ...
  const btn = document.getElementById("publishPost") || document.querySelector(".post-btn");
  if (btn) {
    btn.disabled = true;
...
document.getElementById("openReelsBtn")?.addEventListener("click", (e) => {
  e.preventDefault();
  e.stopPropagation();
  openReels();
});
document.addEventListener("click", (e) => {
  if (e.target.closest("#openReelsBtn")) {
    e.preventDefault();
    openReels();
  }
});
document.getElementById("reelsCloseBtn")?.addEventListener("click", closeReels);
document.getElementById("reelsCreateBtn")?.addEventListener("click", goCreateReel);
document.getElementById("reelsEmptyCreate")?.addEventListener("click", goCreateReel);
...
(function hookPublish() {
  const btn = document.getElementById("publishPost") || document.getElementById("postBtn");
  ...
  btn.addEventListener("click", (e) => {
    if (state.createMode === "reel") {
      ...
      // use media input as video source if reel input empty
      const reelIn = document.getElementById("reelVideoInput");
      const mediaIn = document.getElementById("mediaInput");
