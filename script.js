/* =========================================================
   ...
   V27 — Story likes/replies + mentions + comment likes + own stories
   ========================================================= */
...
async function loadPostComments(post) {
  ...
  // auto-expand reply threads so replies show indented
  rows.forEach((c) => {
    if (c.parent_id) {
      state.openReplyThreads.add(`thread-${c.parent_id}`);
    }
  });
}
