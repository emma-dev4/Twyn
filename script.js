  ...
  try {
    ...
    if (!silent) {
      ...
      if (remoteOk) {
        ...
        alert(
          "Permission is on, but server push may still be failing.\n\n" +
          "Redeploy the send-push Edge Function with the code in send-push-edge.ts\n" +
          "and set VAPID_PRIVATE_KEY + VAPID_PUBLIC_KEY secrets."
        );
      }
    }
    return true;
...
async function sendPushToUser(userId, title, body, url = "./", opts = {}) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.warn("send-push failed", res.status, t);
      return false;
    }
    try {
      const j = await res.json();
      if (j && typeof j.sent === "number" && j.sent < 1) {
        console.warn("send-push: no subscriptions for user", userId, j);
        return false;
      }
    } catch {}
    return true;
