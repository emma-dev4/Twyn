...
async function enablePushNotifications(options = {}) {
  ...
  try {
    ...
    if (!("PushManager" in window) || !("Notification" in window)) {
      if (!silent) {
        alert(
          ...
          "iPhone: use Safari → Share → Add to Home Screen, then open Twyn from the icon.\\n" +
          "Android: use Chrome."
        );
      }
      return false;
    ...
    // iOS only supports Web Push from Home Screen (standalone)
    ...
    if (isIOS && !standalone) {
      if (!silent) {
        alert(
          "On iPhone, notifications only work after you add Twyn to your Home Screen.\\n\\n" +
          "1. Open Twyn in Safari\\n" +
          "2. Tap Share → Add to Home Screen\\n" +
          "3. Open Twyn from the new icon\\n" +
    ...
    if (Notification.permission === "denied") {
      if (!silent) {
        alert(
          "Notifications are blocked for Twyn.\\n\\n" +
          "iPhone: Settings → Notifications → Twyn → Allow Notifications\\n\\n" +
    ...
    // Must request permission from a user gesture when possible
    ...
    // Save to Supabase — try upsert, then insert
    let saveErr = null;
    {
    ...
    if (!silent) {
      ...
      // Also hit edge function so real push pipeline is tested
      try {
        await sendPushToUser(currentUser.id, "Twyn", "Push is working on this phone ✓", "./", { allowSelf: true });
      } catch {}
      alert("Notifications enabled ✓\n\nIf you only saw the in-app banner, check Edge Function logs / VAPID private key.");
    }
    return true;
  } catch (err) {
    ...
    if (!silent) {
      alert(
        (err && err.message) ||
          "Could not enable push. Check push_subscriptions table + VAPID + send-push function."
      );
    }
    return false;
  }
...
async function sendPushToUser(userId, title, body, url = "./", opts = {}) {
  ...
  if (!opts.allowSelf && String(userId) === String(currentUser.id)) return;
  try {
