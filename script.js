...
function renderOpenConnectStrip() {
  const strip = document.getElementById("openConnectStrip");
  if (!strip) return;
  ...
  if (!openPeople.length) {
    strip.innerHTML = `
      <div class="oc-strip-empty">
        <strong>Open Connect</strong>
        <span>Only on Twyn: people who turned on “Open to connect” can be messaged without a phone number. Turn yours on in Profile.</span>
      </div>`;
    return;
  }
  strip.innerHTML = `
    <div class="oc-strip-head">
      <strong>Open Connect</strong>
      <span>Message without numbers · Twyn only</span>
    </div>
    <div class="oc-strip-row" id="ocStripRow"></div>`;
  const row = strip.querySelector("#ocStripRow");
  openPeople.forEach((p) => {
    const el = document.createElement("button");
    el.type = "button";
    el.className = "oc-chip";
    const tags = (p.helpWith || "").split(",").filter(Boolean).slice(0, 2).join(" · ");
    el.innerHTML = `
      <div class="oc-chip-av">${p.avatarUrl ? `<img src="${escapeAttribute(p.avatarUrl)}" alt="">` : escapeHTML((p.name || "?")[0])}</div>
      <div class="oc-chip-meta">
        <b>${escapeHTML(p.name || "User")}</b>
        <small>${tags ? escapeHTML(tags) : "Open to talk"}</small>
      </div>
      <span class="oc-chip-go">Connect</span>`;
    el.addEventListener("click", () => twynConnectTo(p.id, p.name));
    row.appendChild(el);
  });
}
...
document.getElementById("homeFindPeopleBtn")?.addEventListener("click", () => {
  document.querySelector('[data-page="friendsPage"]')?.click();
  setTimeout(() => {
    document.querySelector('.friend-tab[data-tab="recommended"]')?.click();
  }, 80);
});

document.getElementById("homeAskHelpBtn")?.addEventListener("click", () => openAskHelp("life"));

document.querySelectorAll("[data-help-topic]").forEach((btn) => {
  btn.addEventListener("click", () => {
    const t = btn.getAttribute("data-help-topic");
    openAskHelp(t === "fun" ? "fun" : t);
  });
});
...
function renderFriends(type = "followers") {
  ...
  // Open Connect first on Discover
  if (type === "recommended") {
    people = [
