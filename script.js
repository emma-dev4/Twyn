...
function finishOnboardingAndEnter({ openLounge = false } = {}) {
  setOnboardingDone();
  hideOnboarding();
  ...
  if (openLounge) {
    ...
    setTimeout(() => {
      document.querySelector('.inbox-tab[data-inbox="messages"]')?.click();
      setTimeout(() => openLounge(), 200);
    }, 150);
  } else {
...
async function sendLoungeMessage() {
  ...
  const btn = document.getElementById("loungeReplyCancel")?.addEventListener("click", () => clearLoungeReply());
document.getElementById("loungeSendBtn");
