...
function renderPostImagesHTML(imageField, { priority = false } = {}) {
  ...
  const prio = priority ? "high" : "low";
  if (imgs.length === 1) {
    const u = imgs[0];
    if (isVideoUrl(u)) {
      return `<div class="post-images imgs-1"><video class="post-media post-video" src="${escapeAttribute(u)}" controls playsinline preload="none"></video></div>`;
    }
    return `<div class="post-images imgs-1"><img class="post-media" src="${escapeAttribute(u)}" alt="Post media" loading="${load}" decoding="async" fetchpriority="${prio}"></div>`;
  }
  const slides = imgs.map((u, i) =>
    `<div class="post-slide${i === 0 ? " active" : ""}"><img class="post-media" src="${escapeAttribute(u)}" alt="Photo ${i + 1}" loading="${i === 0 && priority ? "eager" : "lazy"}" decoding="async" fetchpriority="${i === 0 && priority ? "high" : "low"}"></div>`
  ).join("");
...
function renderFeed() {
  ...
  // paint first posts now (smooth), rest in chunks so TYP doesn't freeze
  const FIRST = 6;
  const first = posts.slice(0, FIRST);
  const rest = posts.slice(FIRST);
  feed.innerHTML = "";
  const frag1 = document.createDocumentFragment();
  first.forEach((post, i) => frag1.appendChild(createPostElement(post, { priority: i < 2 })));
  ...
  if (rest.length) {
    ...
    const pump = () => {
      ...
      if (offset < rest.length) {
        if (window.requestIdleCallback) requestIdleCallback(pump, { timeout: 200 });
