/* =========================================================
   ...
   V38 — Restore posts/chats load + story tap fix
   ========================================================= */
...
function verifiedBadge(isVerified) {
  return isVerified ? `<span class="verified-badge" title="Verified" aria-label="Verified"></span>` : "";
}
...
function renderPostImagesHTML(imageField) {
  const imgs = parsePostImages(imageField);
  if (!imgs.length) return "";
  const cls = "imgs-" + Math.min(imgs.length, 4);
  return `<div class="post-images ${cls}">${imgs.slice(0,4).map((u) => `<img class="post-media" src="${escapeAttribute(u)}" alt="Post media" loading="lazy">`).join("")}</div>`;
}
...
async function enablePushNotifications(options = {}) {
  ...
  try {
    if (!("serviceWorker" in navigator)) {
      if (!silent) alert("This browser does not support notifications.");
      return false;
    }
    if (!currentUser) return false;

    // iOS requires Home Screen / standalone for Web Push
    const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1);
    const standalone = window.matchMedia("(display-mode: standalone)").matches || window.navigator.standalone === true;
    if (isIOS && !standalone) {
      if (!silent) {
        alert(
          "On iPhone: Add Twyn to your Home Screen first, then open it from the icon and allow notifications.\\n\\nSafari → Share → Add to Home Screen."
        );
      }
      return false;
    }
    ...
    if (!("Notification" in window) || !("PushManager" in window)) {
      if (!silent) alert("Push notifications are not supported on this device.");
      return false;
    }
    ...
    if (!sub) {
      if (!VAPID_PUBLIC_KEY) {
        ...
        if (!silent) alert("Push is not fully configured (missing VAPID key).");
        return false;
      }
    ...
    if (!silent) alert(err.message || "Could not enable notifications.");
    return false;
  }
}
...
async function loadPosts({ reset = false } = {}) {
  ...
  try {
    ...
    if (error) {
      ...
      // Fallback 1: without parent_id on comments
      let fallback = await supabaseClient
        ...
        .range(from, to);
      if (fallback.error) {
        console.error("Posts fallback1:", fallback.error);
        // Fallback 2: plain posts only (always works if RLS allows select)
        ...
        // attach profiles in a second query
...
function openPostFromNotification(postId, commentId) {
  if (!postId) return;
  document.querySelector('[data-page="homePage"]')?.click();
  const tryOpen = () => {
    const post = state.posts.find((p) => String(p.id) === String(postId));
    if (!post) {
      // load feed then retry once
      loadPosts({ reset: true }).then(() => {
        const p2 = state.posts.find((p) => String(p.id) === String(postId));
        if (!p2) return;
        state.openComments.add(String(postId));
        updatePostCardUI(postId);
        const el = document.querySelector(`.post[data-id="${CSS.escape(String(postId))}"]`);
...
function renderInbox() {
  ...
  if (!state.notifications.length) {
    list.innerHTML = `
      <div class="empty-state">
        ...
        <span>Likes, comments, follows, tags, and messages will show up here.</span>
      </div>`;
    return;
  }
  ...
  list.innerHTML = state.notifications
    .map((n) => {
      ...
      if (n.type === "like") {
        ...
        icon = "🎉";
        text = `<strong>${escapeHTML(n.user)}</strong> joined Twyn from your invite`;
      ...
      return `
        <div class="notification ${isUnread ? "unread" : ""}" data-notif-type="${escapeAttribute(n.type)}" data-user-id="${escapeAttribute(n.userId || "")}" data-post-id="${escapeAttribute(n.postId || "")}" data-comment-id="${escapeAttribute(n.commentId || "")}">
  ...
  list.querySelectorAll(".notification").forEach((el) => {
    el.addEventListener("click", () => {
      ...
      } else if ((type === "comment" || type === "mention") && postId) {
        openPostFromNotification(postId, commentId);
      } else if (type === "follow" && userId) {
        openUserProfile(userId);
      } else if (type === "invite_join" && userId) {
        openUserProfile(userId);
      } else if (postId) {
        openPostFromNotification(postId);
      } else if (userId) {
...
async function loadConversations() {
  ...
  try {
    ...
    // merge known people + fetch missing profiles
    const known = {};
...
function setupMessageSeenRealtime(otherUserId) {
  try {
    const ch = supabaseClient
      .channel("seen-" + otherUserId)
      .on(
        "postgres_changes",
        { event: "UPDATE", schema: "public", table: "messages" },
        (payload) => {
          const row = payload.new;
          if (!row || !activeChatUserId) return;
          if (String(row.sender_id) !== String(currentUser?.id)) return;
          if (String(row.receiver_id) !== String(activeChatUserId)) return;
...
function renderChatMessages(userId) {
  ...
  box.innerHTML = list
    .map((m) => {
      ...
      const ticks = mine ? (m.seen_at ? `<span class="chat-ticks read">✓✓</span>` : `<span class="chat-ticks">✓</span>`) : "";
...
/* ========== REALTIME ========== */
let feedAutoReloadTimer = null;
function startSoftAutoReload() {
  if (feedAutoReloadTimer) clearInterval(feedAutoReloadTimer);
  feedAutoReloadTimer = setInterval(() => {
    if (document.hidden) return;
    const home = document.getElementById("homePage");
    if (home && !home.classList.contains("hidden") && !home.classList.contains("active") === false) {
      // quiet refresh without wiping UI if empty error
      loadPosts({ reset: true }).then(() => renderFeed()).catch(() => {});
    }
    if (activeChatUserId) {
      // quiet message refresh only
      supabaseClient
        .from("messages")
        .select("id, sender_id, receiver_id, content, media_url, media_type, created_at, seen_at, reply_to_id, edited_at")
...
function renderStories() {
  ...
  if (addBtn) {
    ...
    // Always open photo picker to ADD a story (reliable tap)
    addBtn.onclick = (e) => {
      e.preventDefault();
      e.stopPropagation();
...
function updateAuthMode() {
  ...
  if (authSwitchBtn) authSwitchBtn.textContent = isLogin ? "Create account" : "Log in";
  if (nameField) nameField.classList.toggle("hidden", isLogin);
  if (authName) authName.required = !isLogin;
  setAuthMessage("");
}

function showAuth() {
  if (authScreen) authScreen.classList.remove("hidden");
  if (twynApp) twynApp.classList.add("hidden");
  document.body.classList.remove("chat-open");
  document.getElementById("chatSpace")?.classList.add("hidden");
  document.getElementById("loungeSpace")?.classList.add("hidden");
  document.getElementById("onboardingScreen")?.classList.add("hidden");
...
function showApp() {
  ...
  if (twynApp) twynApp.classList.remove("hidden");
}
...
authForm?.addEventListener("submit", async (e) => {
  ...
  if (authMode === "signup" && !displayName) {
    setAuthMessage("Enter your name.", true);
    return;
  }
  ...
  try {
    ...
    console.error(err);
    setAuthMessage(err.message || "Auth failed. Try again.", true);
  } finally {
    if (authSubmit) {
