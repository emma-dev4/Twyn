/* =========================================================
   ...
   V30 — Twyn chat colors + visible reply/edit/delete + multi stories
   ========================================================= */
    ...
    .map((m) => {
      ...
      if (m.reply_to_id) {
        ...
        if (parent) {
          ...
          const pText = parent.content || (parent.media_type === "image" ? "Photo" : parent.media_type === "audio" ? "Voice note" : "Message");
          quote = `<div class="chat-reply-quote"><strong>${escapeHTML(pName)}</strong><span>${escapeHTML(String(pText).slice(0, 90))}</span></div>`;
        } else {
          quote = `<div class="chat-reply-quote"><strong>Reply</strong><span>Original message</span></div>`;
        }
      }
      let body = "";
      ...
      const actions = mine
        ? `<div class="chat-bubble-actions">
            <button type="button" data-chat-act="reply" data-msg-id="${escapeAttribute(m.id)}">Reply</button>
            <button type="button" data-chat-act="edit" data-msg-id="${escapeAttribute(m.id)}">Edit</button>
            <button type="button" data-chat-act="delete" data-msg-id="${escapeAttribute(m.id)}">Delete</button>
          </div>`
        : `<div class="chat-bubble-actions">
            <button type="button" data-chat-act="reply" data-msg-id="${escapeAttribute(m.id)}">Reply</button>
          </div>`;

      return `<div class="chat-bubble ${mine ? "mine" : "theirs"}" data-msg-id="${escapeAttribute(m.id)}">
        ${quote}${body}
...
// Visible Reply / Edit / Delete on each bubble
document.getElementById("chatMessages")?.addEventListener("click", (e) => {
  ...
  if (!id) return;
  if (act === "reply") setReplyTo(id);
  else if (act === "edit") startEditMessage(id);
  else if (act === "delete") deleteChatMessage(id);
});
