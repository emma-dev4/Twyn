...
if (imageInput) {
  imageInput.addEventListener("change", (e) => {
    ...
    if (!files.length) return alert("Please choose image or video files.");
...
async function publishReel() {
  ...
  // also check imageInput if reel input empty
  if (!file) {
    const imgIn = document.getElementById("imageInput");
    file = imgIn?.files?.[0];
  }
  ...
  if (!fileLooksLikeVideo(file)) return alert("Please choose a video file (mp4/mov).");
  ...
  try {
    ...
    if (input) input.value = "";
    document.getElementById("createPreview") && (document.getElementById("createPreview").innerHTML = "");
    alert("Reel posted ✓");
    document.querySelector('[data-page="homePage"]')?.click();
    openReels();
  } catch (e) {
    console.error(e);
    alert((e.message || "Could not post Reel") + "\n\nRun the reels SQL in Supabase if the table is missing.");
  } finally {
    if (btn) {
      btn.disabled = false;
      btn.textContent = "Post";
