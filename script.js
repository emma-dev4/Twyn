...
function renderPostImagesHTML(imageField) {
  ...
  if (imgs.length === 1) {
    ...
    return `<div class="post-images imgs-1"><img class="post-media" src="${escapeAttribute(u)}" alt="Post media" loading="lazy"></div>`;
  }
  const slides = imgs.map((u, i) =>
    `<div class="post-slide${i === 0 ? " active" : ""}"><img class="post-media" src="${escapeAttribute(u)}" alt="Photo ${i + 1}" loading="lazy"></div>`
  ).join("");
...
function renderProfile() {
  if (!profileFeed) return;
  updateProfileUI();
  const allMine = state.posts.filter((p) => String(p.userId) === String(currentUser?.id));
...
async function hydrateReels(reels) {
  ...
  if (currentUser && state.reels.length) {
    try {
...
function renderReelsFeed() {
  ...
  viewport.innerHTML = state.reels.map((r, i) => {
    const author = r.original?.profiles || r.profiles;
    ...
    const videoSrc = r.original?.video_url || r.video_url;
    const caption = r.original?.caption || r.caption || "";
    const likes = r.like_count || 0;
    const comments = r.comment_count || 0;
    const isRepost = !!r.repost_of;
    const reposterName = poster?.display_name || poster?.username || "Someone";
    return `
      <div class="reel-slide" data-reel-id="${escapeAttribute(r.id)}" data-reel-index="${i}" data-video-src="${escapeAttribute(videoSrc)}">
        ...
        ></video>
        ${isRepost ? `<div class="reel-repost-label">🔁 ${escapeHTML(reposterName)} reposted</div>` : ""}
        ...
        <div class="reel-actions">
          ...
          <button type="button" class="reel-action-btn" data-reel-repost="${escapeAttribute(r.id)}" title="Repost">
            <span class="reel-ico">🔁</span>
          </button>
...
async function repostReel(reelId) {
  ...
  const reel = state.reels.find((r) => String(r.id) === String(reelId));
  if (!reel) return;
  if (String(reel.user_id) === String(currentUser.id) && !reel.repost_of) {
    // still allow? skip own original
  }
  const originalId = reel.repost_of || reel.id;
  try {
    const { error } = await supabaseClient.from("reels").insert({
      user_id: currentUser.id,
      video_url: reel.original?.video_url || reel.video_url,
      caption: reel.original?.caption || reel.caption || "",
      like_count: 0,
      view_count: 0,
    ...
    if (error) {
      // schema without repost_of
      if (String(error.message || "").includes("repost_of") || error.code === "PGRST204") {
