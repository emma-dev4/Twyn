...
function createPostElement(post) {
  ...
  article.innerHTML = `
    ...
    ${post.image ? renderPostImagesHTML(post.image) : ""}
...
function openReelMenu(reelId) {
  ...
  if (postOptionsModal) {
    ...
    postOptionsModal.style.zIndex = "600";
...
async function toggleReelLike(reelId) {
  ...
  try {
    if (reel.liked) {
      ...
      await supabaseClient.from("reel_likes").insert({ user_id: currentUser.id, reel_id: reelId });
      reel.liked = true;
