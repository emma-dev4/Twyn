...
function renderReelsFeed() {
  ...
  setTimeout(() => playReelAt(0), 300);
  setTimeout(() => playReelAt(0), 800);
}

function ensureReelVideoReady(video, slide) {
  if (!video) return;
  // iOS + Android autoplay rules: must be muted + playsinline
  ...
  // never set crossOrigin on mobile media buckets — breaks play
  ...
  if (!video.dataset.errBound) {
    ...
    video.addEventListener("error", () => {
      ...
      tip.textContent = "Can't play this file. Re-upload as MP4 (H.264).";
...
function showReelPlayButton(slide, video) {
  ...
  if (!tip) {
    ...
    tip.style.cssText = [
      ...
      "background:rgba(255,255,255,.92)","color:#111","font-size:28px",
  ...
  const doPlay = (ev) => {
    ...
    if (p && p.catch) {
      p.catch(() => {
        tip.innerHTML = "!";
        tip.title = "Can't play this file";
      });
    }
    const hide = () => { try { tip.remove(); } catch {} };
    video.addEventListener("playing", hide, { once: true });
    setTimeout(() => { if (!video.paused) hide(); }, 500);
  };
...
function playReelAt(index) {
  ...
  slides.forEach((slide, i) => {
    ...
    if (i === index) {
      ...
      setTimeout(() => {
        if (video.paused) showReelPlayButton(slide, video);
      }, 400);
    } else {
