...
function renderPostImagesHTML(imageField) {
  ...
  if (imgs.length === 1) {
    return `<div class="post-images imgs-1"><img class="post-media" src="${escapeAttribute(imgs[0])}" alt="Post media" loading="lazy"></div>`;
  }
...
if (imageInput) {
  imageInput.addEventListener("change", (e) => {
    ...
    if (!files.length) return alert("Please choose image files.");
    if (files.some((f) => f.size > 5 * 1024 * 1024)) return alert("Each image must be under 5MB.");
    selectedImages = files.slice(0, 4);
...
document.getElementById("publishBtn")?.addEventListener("click", async () => {
  ...
  if (!text && !selectedImages.length && !selectedImage) return alert("Write something or add a photo.");
...
async function loadReels() {
  ...
  try {
    const { data, error } = await supabaseClient
      .from("reels")
      .select(`
        id, user_id, video_url, caption, like_count, view_count, created_at,
    ...
    if (empty) {
      ...
      empty.querySelector("strong").textContent = "Couldn’t load Reels";
      empty.querySelector("span").textContent = e.message || "Create the reels table in Supabase (see SQL).";
    }
  }
}
