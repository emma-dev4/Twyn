...
function nameBadges(opts = {}) {
  // OG badge removed — only verified + founder
...
function renderProfile() {
  ...
  let myPosts = allMine;
  if (tab === "reposts") {
    myPosts = allMine.filter((p) => p.repostOf || /^🔁\s*Repost/i.test(String(p.text || "")));
  } else if (tab === "media") {
    myPosts = allMine.filter((p) => !!p.image);
  } else {
    myPosts = allMine.filter((p) => !p.repostOf && !/^🔁\s*Repost/i.test(String(p.text || "")));
  }
  ...
  if (!myPosts.length) {
    if (tab === "reposts") {
      profileFeed.innerHTML = `<div class="empty-state"><div class="empty-icon">🔁</div><strong>No reposts yet</strong><span>When you repost someone, it shows up here.</span></div>`;
    } else if (tab === "media") {
      profileFeed.innerHTML = `<div class="empty-state"><div class="empty-icon">🖼️</div><strong>No media yet</strong><span>Posts with photos will appear here.</span></div>`;
    } else {
      profileFeed.innerHTML = `<div class="empty-state"><div class="empty-icon">✍️</div><strong>You haven’t posted yet</strong><span>Share your first post and it will appear here.</span><button class="empty-action" onclick="document.querySelector('[data-page=\\'createPage\\']')?.click()">Create a post</button></div>`;
    }
    return;
  }
