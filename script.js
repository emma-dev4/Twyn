  ...
  try {
    ...
    box.innerHTML = rows
      .map((m) => {
        ...
        const time = formatChatClock(m.created_at);
        return `<div class="chat-bubble ${mine ? "mine" : "theirs"}" data-lounge-id="${escapeAttribute(m.id)}" data-lounge-name="${escapeAttribute((prof.username || prof.display_name || "").toString().split(" ")[0])}">
...
(function setupLoungeGestures() {
  ...
  box.addEventListener("click", (e) => {
    ...
    if (!bubble) return;
    if (bubble.classList.contains("lounge-system")) return;
