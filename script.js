...
function renderPostImagesHTML(imageField) {
  ...
  if (imgs.length === 1) {
    ...
    return `<div class="post-images imgs-1"><img class="post-media" src="${escapeAttribute(u)}" alt="Post media" loading="lazy" decoding="async" fetchpriority="low" decoding="async"></div>`;
  }
  const slides = imgs.map((u, i) =>
    `<div class="post-slide${i === 0 ? " active" : ""}"><img class="post-media" src="${escapeAttribute(u)}" alt="Photo ${i + 1}" loading="lazy" decoding="async"></div>`
  ).join("");
...
/* ========== STORAGE ========== */
async function compressImageForUpload(file, maxSide = 1280, quality = 0.78) {
  try {
...
function renderFeed() {
  ...
  const frag = document.createDocumentFragment();
  posts.forEach((post) => frag.appendChild(createPostElement(post)));
  feed.innerHTML = "";
...
function createPostElement(post) {
  ...
  article.innerHTML = `
    ...
    ${post.image ? renderPostImagesHTML(post.image) : (!post.text ? `<div class="post-text" style="color:var(--muted);font-size:13px">Media unavailable</div>` : "")}
