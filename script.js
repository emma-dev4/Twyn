...
function renderPostImagesHTML(imageField) {
  ...
  if (imgs.length === 1) {
    ...
    return `<div class="post-images imgs-1"><img class="post-media" src="${escapeAttribute(u)}" alt="Post media" loading="lazy" decoding="async" decoding="async"></div>`;
  }
...
function showReelPlayButton(slide, video) {
  ...
  if (!tip) {
    ...
    tip.innerHTML = `<span class="reel-play-icon">▶</span>`;
    tip.style.cssText = [
...
async function toggleReelLike(reelId) {
  ...
  try {
    ...
    if (btn) {
      ...
      if (ico) ico.textContent = reel.liked ? "♥" : "♡";
    ...
    sheet.innerHTML = `
      ...
      <div class="reel-comments-panel">
        <div class="reel-comments-head">
          ...
          <button type="button" data-close-reel-comments>✕</button>
        </div>
...
async function loadReelComments(reelId) {
  ...
  try {
    ...
    list.innerHTML = rows.map((c) => {
      ...
      return `<div class="reel-comment-row">
        <strong>${escapeHTML(name)}</strong>
        <span>${escapeHTML(c.content || "")}</span>
      </div>`;
    }).join("");
  } catch (e) {
    list.innerHTML = `<div class="empty-state" style="padding:20px"><span>${escapeHTML(e.message || "Comments need reels-extra.sql")}</span></div>`;
  }
}
