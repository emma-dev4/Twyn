...
function openReels() {
  try {
    ...
    if (!space) {
      console.error("reelsSpace missing");
...
async function loadReels() {
  ...
  if (!viewport) return;
  viewport.innerHTML = `<div class="reel-slide" style="color:#fff;place-content:center;display:grid">Loading Reels…</div>`;
  try {
    // No join — FK to profiles may not exist in schema cache
    const { data, error } = await supabaseClient
      .from("reels")
    ...
    if (empty) {
      ...
      if (span) span.textContent = e.message || "Run reels.sql in Supabase if the table is missing.";
    }
  }
}
...
function renderReelsFeed() {
  ...
  viewport.innerHTML = state.reels.map((r, i) => {
    const name = r.profiles?.display_name || "Twyn user";
    const uname = r.profiles?.username ? "@" + r.profiles.username : "";
    const av = r.profiles?.avatar_url
      ? `<img src="${escapeAttribute(r.profiles.avatar_url)}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`
      : escapeHTML((name || "T")[0].toUpperCase());
    const likes = r.like_count || 0;
    return `
      <div class="reel-slide" data-reel-id="${escapeAttribute(r.id)}" data-reel-index="${i}">
        <video src="${escapeAttribute(r.video_url)}" playsinline loop muted preload="${i < 3 ? "auto" : "metadata"}"></video>
        <div class="reel-overlay">
          <div class="reel-user" data-open-profile="${escapeAttribute(r.user_id)}">
            <div class="avatar">${av}</div>
            <div>
              <div>${escapeHTML(name)}${r.profiles?.is_verified ? " ✓" : ""}</div>
        ...
        <div class="reel-actions">
          <button type="button" class="reel-action-btn ${r.liked ? "liked" : ""}" data-reel-like="${escapeAttribute(r.id)}">
            ${r.liked ? "♥" : "♡"}<span>${likes}</span>
          </button>
          <button type="button" class="reel-action-btn" data-reel-mute="${escapeAttribute(r.id)}">🔊</button>
        </div>
      </div>`;
  }).join("");
  ...
  setupReelsObserver();
  // play first
  const first = viewport.querySelector("video");
  if (first) {
    first.muted = true;
    first.play().catch(() => {});
  }
...
function setupReelsObserver() {
  ...
  reelsObserver = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        if (entry.isIntersecting && entry.intersectionRatio > 0.55) {
          document.querySelectorAll("#reelsViewport video").forEach((v) => {
            if (v !== video) {
              try { v.pause(); v.currentTime = v.currentTime; } catch {}
            }
          });
          const p = video.play();
          ...
          if (id && !entry.target.dataset.viewed) {
            ...
            supabaseClient.rpc("increment_reel_views", { reel_id: id }).catch(() => {
              supabaseClient.from("reels").update({ view_count: (parseInt(entry.target.dataset.views || "0", 10) + 1) }).eq("id", id).then(() => {});
            });
          }
        } else {
    ...
    { root: document.getElementById("reelsViewport"), threshold: [0.6] }
  );
...
async function toggleReelLike(reelId) {
  ...
  try {
    ...
    if (btn) {
      ...
      btn.innerHTML = `${reel.liked ? "♥" : "♡"}<span>${reel.like_count || 0}</span>`;
    }
  } catch (e) {
...
function setCreateMode(mode) {
  ...
  if (tools) {
    ...
    hint.textContent =
      state.createMode === "reel"
        ? "Reel mode: tap Photo and pick a short video (mp4/mov, under ~40MB)."
        : "";
  }
}
...
async function publishReel() {
  if (!currentUser) return alert("Please log in first.");
  // Prefer in-memory selection (iOS often clears input.files after change)
  let file =
  ...
  if (btn) {
    btn.disabled = true;
    btn.textContent = "Uploading Reel…";
  }
  try {
    ...
    if (error) {
      throw new Error(
        (error.message || "Insert failed") +
          (String(error.message || "").includes("relation") || String(error.code) === "42P01"
            ? " — run reels.sql in Supabase SQL Editor"
            : "")
      );
    }
    ...
    const reelIn = document.getElementById("reelVideoInput");
    if (reelIn) reelIn.value = "";
...
// re-bind after app shows (auth flow)
const _showAppOrig = typeof showApp === "function" ? showApp : null;
...
document.addEventListener("click", (e) => {
  const like = e.target.closest("[data-reel-like]");
  if (like) {
    toggleReelLike(like.dataset.reelLike);
    return;
  }
  const mute = e.target.closest("[data-reel-mute]");
  if (mute) {
  ...
  if (prof && document.getElementById("reelsSpace") && !document.getElementById("reelsSpace").classList.contains("hidden")) {
...
// Hook publish button: if reel mode, publish reel
  ...
  btn.addEventListener(
    ...
    (e) => {
      if (state.createMode === "reel") {
        ...
        const reelIn = document.getElementById("reelVideoInput");
        const mediaIn = document.getElementById("imageInput");
        if (reelIn && mediaIn && mediaIn.files?.length && !reelIn.files?.length) {
          try {
            const dt = new DataTransfer();
            dt.items.add(mediaIn.files[0]);
            reelIn.files = dt.files;
          } catch {}
        }
        publishReel();
      }
