...
function waveKey() {
  return "twyn_wave_" + (currentUser?.id || "x") + "_" + new Date().toISOString().slice(0, 10);
}

function renderTwynWaveCard() {
  const card = document.getElementById("twynWaveCard");
  if (!card || !currentUser) return;
  let used = false;
  try { used = localStorage.getItem(waveKey()) === "1"; } catch {}
  const openPeople = (state.people || []).filter(
    (p) => p.openToConnect && String(p.id) !== String(currentUser.id)
  );
  if (!openPeople.length) {
    card.innerHTML = "";
    card.classList.add("hidden");
    return;
  }
  card.classList.remove("hidden");
  if (used) {
    card.innerHTML = `<strong>Twyn Wave</strong><span>You already sent today’s wave. Come back tomorrow.</span>`;
    return;
  }
  const pick = openPeople[Math.floor(Math.random() * openPeople.length)];
  card.innerHTML = `
    <div class="wave-top"><strong>Twyn Wave</strong><span>Only on Twyn · once a day</span></div>
    <p>Send a quick wave to <b>${escapeHTML(pick.name || "someone")}</b> — no number, no long intro.</p>
    <button type="button" id="sendTwynWave">Wave 👋</button>`;
  document.getElementById("sendTwynWave")?.addEventListener("click", async () => {
    try {
      await twynConnectTo(pick.id, pick.name);
      setTimeout(() => {
        const input = document.getElementById("messageInput") || document.getElementById("chatInput");
        if (input) input.value = "Wave 👋 from Twyn";
      }, 350);
      try { localStorage.setItem(waveKey(), "1"); } catch {}
      renderTwynWaveCard();
    } catch (e) {
    ...
    if (error) {
      ...
      if (fallback.error) {
        ...
        fallback = await supabaseClient
          .from("posts")
          .select("id, user_id, content, image_url, world, created_at")
          .order("created_at", { ascending: false })
          .range(from, to);
        if (fallback.error) {
