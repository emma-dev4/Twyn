...
function renderReelsFeed() {
  ...
  viewport.innerHTML = state.reels.map((r, i) => {
    ...
    return `
      <div class="reel-slide" data-reel-id="${escapeAttribute(r.id)}" data-reel-index="${i}" data-video-src="${escapeAttribute(videoSrc)}">
        ...
        <button type="button" class="reel-menu-btn" data-reel-menu="${escapeAttribute(r.id)}" aria-label="Options" style="position:absolute;top:calc(12px + env(safe-area-inset-top,0px));right:56px;z-index:10;width:40px;height:40px;border-radius:50%;border:0;background:rgba(0,0,0,.35);color:#fff;font-size:22px;line-height:1;backdrop-filter:blur(6px);cursor:pointer">⋯</button>
  ...
  viewport.querySelectorAll(".reel-slide").forEach((slide) => {
    ...
    slide.addEventListener("click", (e) => {
      if (e.target.closest(".reel-actions, .reel-user, .reel-tap-play, button, a")) return;
...
function showReelPlayButton(slide, video) {
  ...
  tip.textContent = "▶ Play";
  tip.onclick = (ev) => {
    ev.preventDefault();
    ev.stopPropagation();
    // iOS: start muted first (always allowed), then optional unmute
    video.muted = true;
    video.defaultMuted = true;
    const go = () => {
      const p = video.play();
      if (p && p.catch) {
        p.catch(() => {
          tip.textContent = "Can't play — use MP4";
        });
      }
    };
    go();
