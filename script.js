...
async function loadReelComments(reelId) {
  ...
  try {
    ...
    const renderOne = (c, isReply) => {
      ...
      return `<div class="reel-comment-row ${isReply ? "is-reply" : ""}" data-rc-id="${escapeAttribute(c.id)}">
        <div class="reel-comment-main">
          <strong>${escapeHTML(name)}</strong>
          <span>${escapeHTML(c.content || "")}</span>
          <div class="reel-comment-actions">
            <button type="button" data-rc-like="${escapeAttribute(c.id)}" class="${liked ? "on" : ""}">${liked ? "Liked" : "Like"}${lc ? " · " + lc : ""}</button>
            <button type="button" data-rc-reply="${escapeAttribute(c.id)}" data-rc-name="${escapeAttribute(name)}">Reply</button>
            ${mine ? `<button type="button" data-rc-del="${escapeAttribute(c.id)}">Delete</button>` : ""}
          </div>
        </div>
        ${(byParent[String(c.id)] || []).map((r) => renderOne(r, true)).join("")}
      </div>`;
    };

    list.innerHTML = tops.map((c) => renderOne(c, false)).join("") ||
      rows.map((c) => renderOne(c, false)).join("");
