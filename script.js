...
function nameBadges(opts = {}) {
  return `${verifiedBadge(!!opts.isVerified)}${founderBadge(!!opts.isFounder)}${ogBadge(!!opts.isOg && !opts.isFounder)}${motwBadge(!!opts.isMotw)}`;
}
...
function renderProfile() {
  ...
  let myPosts = allMine;
  if (tab === "reposts") myPosts = allMine.filter((p) => p.repostOf || /^🔁\s*Repost/i.test(p.text || ""));
  else if (tab === "media") myPosts = allMine.filter((p) => !!p.image);
  else myPosts = allMine.filter((p) => !p.repostOf && !/^🔁\s*Repost/i.test(p.text || ""));
  const postCount = document.getElementById("postCount");
  if (postCount) postCount.textContent = (typeof allMine !== "undefined" ? allMine : myPosts).length;
  loadFollowCounts();
...
async function repostPost(postId) {
  ...
  if (String(original.userId) === String(currentUser.id) && !original.repostOf) {
    if (!confirm("Repost your own post?")) return;
  }
  try {
    const payload = {
      user_id: currentUser.id,
      content: original.text || "",
      image_url: original.image || null,
      world: original.world || "general",
      repost_of: original.repostOf || original.id
    };
    let { data, error } = await supabaseClient.from("posts").insert(payload).select("id").single();
    if (error && /repost_of|schema cache|column/i.test(error.message || "")) {
      ...
      ({ data, error } = await supabaseClient.from("posts").insert({
        user_id: currentUser.id,
        content: quote,
        image_url: original.image || null,
        world: original.world || "general"
